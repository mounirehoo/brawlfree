$(document).ready(function() {
	...
	progressBarResource2(10, $('#progressBarResource2'));
		...
		change: function(event, ui) { 
				...
				$('#resource-2-amount').html(sliderValueResource2);
					if(sliderValueResource2 == '1000') {
						...
						progressBarResource2(30, $('#progressBarResource2'));
						$('#decrease-resource-2').removeClass('btn-disabled');
					}
						...
						progressBarResource2(50, $('#progressBarResource2'));
						$('#increase-resource-2').removeClass('btn-disabled');
					}
					else if (sliderValueResource2 == '6000') {
						progressBarResource2(60, $('#progressBarResource2'));
						$('#increase-resource-2').addClass('btn-disabled');
					}
					else if (sliderValueResource2 == '7000') {
						progressBarResource2(70, $('#progressBarResource2'));
						$('#increase-resource-2').addClass('btn-disabled');
					}
					else if (sliderValueResource2 == '8000') {
						progressBarResource2(80, $('#progressBarResource2'));
						$('#increase-resource-2').addClass('btn-disabled');
					}
					else if (sliderValueResource2 == '9000') {
						progressBarResource2(90, $('#progressBarResource2'));
						$('#increase-resource-2').addClass('btn-disabled');
					}
					else if (sliderValueResource2 == '10000') {
	...
	$('#second-step-button').click(function() {	
		...
		progressBar(0, $('#progressBarConsole'));
		$('.message-header h3').html("Brawl Stars Generator");	
		$('.message-header p.message-header-subtitle').html("You are about to generate Coins and Gems");		
		$(".message-content p.console-message").fadeIn();	
		var $console_message_username_msg = $('#coc-player-tag').val();
		var $console_message_platform_msg = $('#coc-player-platform').val();
...
var ChatContent = ["How much brawl stars coins can I generate?", "Anyone tried this already?", "Does it work in NA?", "Why this is so easy lol?", "This is incredible, never thought it would work.", "I generated 10000 gems, can't wait to start.", "ios player here, works flawless.", "Can someone help me with the survey?", "OMG!", "LOL!", "ROFL!", "Real", "haha", "easy", "bro", "What can I do here?", "Shut up man I love this website", "hi guys", "How much gems u made so far?", "what about surveys on mobile phone?", "Is this free?", "How long do you have to wait?", "Yea", "No", "I know", "Exactly why this is so good", "uhm", "maybe", "I can't wait anymoreeee", "Is this for real guys?", "Thanks man I appreciate this.", "Cool =)", "<message deleted>", "oh god", "damn", "I love this", "Never imagined this would work but damn its so simple", "saw this on forums pretty impressive", "yo guys dont spam okay?", "anyone up for a game?", "you think this will be patched any time soon", "pretty sure this is saving me a lot of money", "any idea how long it takes for Coins and gems to come?", "so happy i found this", "you guys watch Moon Stars?", "I have seen this website on youtube vid stream i think", "just wow", "Where do I get my Coins and gems?", "a friend told me about this", "thanks to whoever spams this website lol", "where i put in my code?", "so far I am cool with this", "can I get for free?", "bye guys", "okay i applied thank you", "how much can you even have", "incredible", "ten minutes", "need to go now", "brb", "You should give it a try", "dont regret being here", "fucking is real", "omg stop asking how to get brawl stars gems just get it from generator", "guys this is so easy, it takes less than a minute", "Can anyone do it for me? My username is brazilinaronaldo", "PM me pls", "shadow fight sucks noobs haha", "EA pls", "today is lucky day", "this is the best brawl stars gems website because we all have more than a chance", "i think everyone here got gems", "when can I play I am new to this", "brawl stars gems for free?", "Do gems expire?", "I got big pack of gems for my girlfriend making her happy and i dont pay for them lol", "man servers are always down fuk it", "funny how this works but it does like always", "hi again im here for more gems", "i need some brawl stars gems what do i do", "this worked lol", "fuck i have no surveys left, had like 50k already on my acc", "where do all of you come from", "nice page for brawl stars gems", "i was stuck in survey had to do again but it worked then", "thank you for giving me gems!", "saw on yt yo", "brawl stars gems working fine", "i love brawl stars so much", "this makes my game more enjoyable i hope", "thank you all for helping me out bros", "thanks to whoever pmed me it worked", "thank you for messaging me man", "when do you wanna play?", "imagine all the people waiting fo this", "any idea if this still works tomorrow", "best gems website", "is this twitch chat?", "wow really many people online here", "hi all who has some gems for me", "anyone not here for brawl stars lol?", "who is up for a chat hehe?", "i play in EU", "check my profile i am rich", "when is Coins start men?", "even noobs can do this", "when did you guys start playing wow", "i can only recommend this stuff",  "can't wait for it to start!", "where do you come from?", "does this giveaway go forever?", "pretty good gems signup form guys", "i begin to like this very much. third pack i unlocked", "worth", "ok cool", "i see no limits on how gems you can get thats so epic", "which country are you playing in guys?", "think so man", "Likely, but I think one day this will fail", "this still works at the moment", "i havent seen this before but im impressed with the result!", "my boyfriend will freak out :D", "nice ", "surveys dont appaer every time but i think its there to have enough money for the website to buy get the gems codes", "actually i had no problem with any survey ever, just try?", "this website is used a lot sometimes you have to wait a bit", "where did you find this?", "so when will gems start?", "ty for the gems opt in guys!", "i wish i found this earlier", "i wasted so much money on brawl stars lol - good this is free here", "how come i dont see any trolls here", "just dodged queue for this", "any bro needs help?", "i would do screenshot but maybe you report me then", "trololo brawl stars sucks hahahaha", "i feel like this will be the best! it was starting to get boring lol", "think so", "what you can get gems here for free?", "ok sounds good enough for me bros", "anyone reddit here?", "Okay I believe this works cus I just logged in and saw my coins and gems ROFL", "I had a bit trouble with some survy thing but no problem if you just choose an easy", "my friends on facebook spam this like every day they are rly happy about it", "Where do i put my phone", "what?", "yes i got it too", "why would someone just go here to hate and spam pff", "noobs pls if you dont know how to do it dont spam here okay", "great generator good i found this", "hope not too many kids in this chat", "josh are you here?", "unlocking takes some time for me", "derp", "i am curious is this legit?", "Works on ios?", "had to reload page before it worked", "used this three times and applied for 3  10.000 coin packs, lol see you ingame suckers", "i see most people here write positive things it is true?", "hi my english no good i here get brawl stars coins?", "Exactly what I think", "you can have reginalds IQ and still be able to get coins", "when i came first to this website i was like most of you guys just spamming here the chat, in the end im glad that i tried it because now for next year or so i am not leaving my room", "if you want a proof add me on skype", "I thought brawl stars is slowly dying, i hope this release will get some players back", "thank you!", "are you not bored at all? i cant wait for expansion pack", "i am looking for a friend please pm me", "i thought my friend wanted to fool me with this website link. but you can rly get coins here if you dont mess up with the survey part", "aasdasdasd", "Ok so I am back and what I can say is that i got my coins! I can not do a screenshot cuz the chat would block any links meh but rly go try it its worth it", "worth got my gems", "i agree", "i am fine with having free gems how about you", "what i always disliked is when you get close to release date and they move it even further", "from all websites ive been on this is the first and probably the only one which rly gives you the brawl stars gems", "i have tried too many surveys in my life finally i got lucky here ", "yeah free gems is cool", "you like this?", "What you think about all this", "I want to play from korea", "wow i waited ages to get a server transfer now here it shouldnt be a problem anymore", "lol ProAsh32 is here? you were in my skype! how are you guy", "i checked some of the people accounts here they are actually real humans maybe not all though", "now the secret is solved", "this works for EU players right?", "hey i am a newbie will i be able to play?", "i signed up, now the waiting starts :/ i hope they will launch sooner", "can i do this with my nexus phone?", "...^^", "fucking hilarious some people", "brawl stars gems here I come", "wow 10 minutes ago this was empty now all people here wtf", "i can imagine that", "okay", "not sure if i understood? its all free right?", "I would be so sad if this did not work because it took a while, thankfully it worked then", "uhm", "so you can buy everything now guys?",  "fucking helll! got my coins!.", "yayy", "servers i tested this and its working", "i usually choose the first offer in the list because its normally the easiest one", "i think some offers easier in countries like USA", "if you chose an offer make sure to finish it complete or you will not sign up for gems guys!"];
